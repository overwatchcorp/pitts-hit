...
var distractors = d3.range(n).map(function() {
	var x = Math.random() * (w - 150) + 50, y = Math.random() * (h - 150) +50,
	    myArray = [1,-1];
	return {
        vx: (Math.random() * (speedMax - speedMin) + speedMin) * myArray[Math.floor(Math.random() * myArray.length)],
        vy: (Math.random() * (speedMax - speedMin) + speedMin) * myArray[Math.floor(Math.random() * myArray.length)],
        //vx: speedMin,
        //vy: speedMin,
...
var svg = d3.select("#svgHere").append("svg:svg")
    .attr("width", w)
    .attr("height", h);
...
// T's and L's
var head = g.append("svg:text")
	.attr("class","head")
	.style("fill",function(d,i) {if (i % 2 == 0){return "black";} else {return "white";}}) //Make half black/white
	.text(function(d,i) {if (i < n/2.0){return "T";} else {return "L";}}) //Make half black/white
...
function makeDistractors(curTime){
	...
	for (var i = -1; ++i < n;) {
    	...
    	if (x < 0 || x > w-27) {distractor.vx *= -1;}//bounces.text(bounceNum+1);bounceNum = bounceNum +1;}
    	if (y < 36 || y > h) {distractor.vy *= -1;}//bounces.text(bounceNum+1);;bounceNum = bounceNum +1;}
	
    	// Count center line passes
    	if (i % 2 == 1){ //count all white ones, not just T's or L's
        	if (curTime < 16000){
        	//if (i < n/2.0){ //turn on for just L's
        	    if (y < (h/2)+v/2.8 && y > (h/2)-v/2.8) {bounces.text(bounceNum+1);bounceNum = bounceNum +1;}
                }
            }	
	    //}
	...
	head.attr("transform", function(d) {return "translate(" + d.path[0] + ")"; });
...
d3.timer(function() {
	...
	if (curTime > endEvent) {
		if (endStarted == 'False') {
            ...
            distpath = $('#distpath');
            output = '';
            for (i in distPathArray){output += distPathArray[i] + ';';};            
...
function animatePleaseType(){
    if (event2started == 'True') {
        d3.selectAll("#typeResp")
         .transition()
         .text('Did you notice anything on this last trial that was different from the first three trials of the experiment? [Y/N]')
         .duration(250)
    	...
    	d3.select("#respInput").node().focus() //this is the same function of the one above it, just in d3
...
function setbg(color){ // Sets styling for question box
